import java.io.*;
import java.util.ArrayList;
import java.util.LinkedList;
import java.util.Queue;

public class A3
{
    public static void main(String[] args)
    {
        int totalNumberOfFrames = Integer.parseInt(args[0]);                int timeQuantum = Integer.parseInt(args[1]);                        int NumOfProcesses = args.length - 2;                               int allocatedFrames = totalNumberOfFrames/NumOfProcesses;   
        Queue<Process> readyQ = new LinkedList<Process>();                  int id = 1;                         int fileDone = 0;                 int fileNumber = 2;              while (fileDone < NumOfProcesses)
        {
            File file = new File(args[fileNumber]);
            try {
                BufferedReader br = new BufferedReader(new FileReader(file));
                String line = br.readLine();
                line = br.readLine();
                ArrayList<Integer> pageRequests = new ArrayList<Integer>();                         while (!line.equals("end"))
                {
                    pageRequests.add(Integer.parseInt(line));                       line = br.readLine();
                }
				
                                Process temp = new Process(id, args[fileNumber].substring(3, args[fileNumber].length()), pageRequests, allocatedFrames);
                                readyQ.add(temp);
                id++;                               fileNumber++;                       fileDone++;         
            } catch (FileNotFoundException e) {
                e.printStackTrace();
            } catch (IOException e) {
                e.printStackTrace();
            }
        }

        System.out.println("LRU - Fixed:");
        Processor algorithm = new Processor(readyQ, timeQuantum);
                ArrayList<Process> output = algorithm.run(true);
        printResults(output);           
        System.out.println("\n-------------------------------------------------------------------------------\n");
        algorithm.reset();
        System.out.println("Clock - Fixed:");
                output = algorithm.run(false);
        printResults(output);           
    }

    
    public static void printResults(ArrayList<Process> output)
    {
        System.out.println("PID\tProcess Name\tTurnaround Time\t# Faults\tFaultTimes");
        for (int i = 1; i < 5; i++)
        {
            for (int z = 0 ; z < output.size(); z++)
            {
                if (output.get(z).getId() == i)
                {
                    System.out.print(output.get(z).getId() + "\t" + output.get(z).getpName() + "\t" + output.get(z).getFinishedTime() + "\t\t" + output.get(z).getFaultTimes().size() + "\t\t{");

                                        ArrayList<Integer> faultTimes = output.get(z).getFaultTimes();
                    for (int q = 0; q < faultTimes.size(); q++)
                    {
                        if (q == faultTimes.size() - 1)
                        {
                            System.out.println(faultTimes.get(q) + "}");
                        }
                        else
                        {
                            System.out.print(faultTimes.get(q) + ", ");
                        }
                    }
                }
            }
        }
    }
}