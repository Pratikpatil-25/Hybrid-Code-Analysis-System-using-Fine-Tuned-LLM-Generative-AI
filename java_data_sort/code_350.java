...
public final class SWITCH implements CompoundInstruction {
  ...
  public SWITCH(int[] match, InstructionHandle[] targets,
    ...
    if((match_length = match.length) < 2)       instruction = new TABLESWITCH(match, targets, target);
    else {
  ...
  private final void sort(int l, int r) {
    ...
    do {
      ...
      if(i <= j) {
	h=match[i]; match[i]=match[j]; match[j]=h; 	h2=targets[i]; targets[i]=targets[j]; targets[j]=h2; 	i++; j--;
      }
    } while(i <= j);
